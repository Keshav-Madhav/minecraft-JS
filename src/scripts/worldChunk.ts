...
import { blockArrayMaterial } from './blockArrayMaterial';
...
export class WorldChunk extends THREE.Group {
  ...
  generate(resources: ResourceGenInfo[], getWorldBlock?: WorldBlockGetter) {
    const data = generateChunkData(this.size, this.params, this.position.x, this.position.z, resources);
    this.setData(data);
    this.buildMeshes(getWorldBlock);
  }
  ...
  buildMeshes(getWorldBlock?: WorldBlockGetter) {
    ...
    const geometry = buildChunkGeometry(this.data, this.size, getOutside);
    this.applyGeometry(geometry.casters, geometry.nonCasters);
  }
  ...
  applyGeometry(casters: GeometryArrays | null, nonCasters: GeometryArrays | null) {
    this.clearMeshes();
    this.addGeometryMesh(casters, true);
    ...
    this.addGeometryMesh(nonCasters, true);
    this.loaded = true;
  }
  ...
  private addGeometryMesh(arrays: GeometryArrays | null, castShadow: boolean) {
    ...
    const mesh = new THREE.Mesh(geometry, blockArrayMaterial);
  ...
  addBlock(x: number, y: number, z: number, id: number, getWorldBlock?: WorldBlockGetter) {
  ...
  removeBlock(x: number, y: number, z: number, getWorldBlock?: WorldBlockGetter) {
    if (this.getBlockId(x, y, z) !== BLOCK_IDS.air) {
